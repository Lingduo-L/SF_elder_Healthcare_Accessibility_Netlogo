extensions [ gis csv ]
globals [
  seniorClinics ;a vector dataset of points for senior clinics locations
  notSeniorClinics ; a vector dataset points for locations that are in the health facility data but are not senior clinics
  censusBlocks ; a vector dataset of polygons for the outline of census blocks
  Sj ;a variable from Guy's (1983) gaussian measure for food accessibility 这个后面可能要改一改？
...
patches-own [
  ...
  population ;the senior population from the census dataset
  foodstamps ;the foodstamps value from the census dataset
  poverty ;the poverty percentage from the census dataset
  disability ;the 1+ persons with diability percentage of household from the census dataset
  ...
  clinic-score ;the accessibility score for each clinic for a unique person...one store can have different store-scores becuase it is also unique to each person
  seniorClinic-here? ;marked TRUE is there is a senior clinic on the patch
  notSeniorClinic-here? ;marked TRUE is there is a health facility that is not a senior clinic on the patch
  cancerValue
  highBloodPressure
  diabetesValue ;我把原wards的field都放到了census blocks data里
]
...
people-own [
  ...
  fs ; 直接用fs
  pov ; 直接用poverty
  ; 除此之外我还加了diability, healthInsurance这两个指标，参看前段，可以选用
  clinic-in-radius ; patch set of patches with seniorClinic-here? = TRUE or notSeniorClinic-here? = TRUE
...
to setup ;load the gis data
  clear-all
  reset-ticks
  gis:load-coordinate-system (word "Data/SFCensusBlocks.prj")
  set seniorClinics gis:load-dataset "Data/SFSeniorHealth.shp" ;;;this is the clinic location data
  set censusBlocks gis:load-dataset "Data/SFCensusBlocks.shp"
  set notSeniorClinics gis:load-dataset "Data/DCNOTSupermarkets.shp"
end
...
to draw ; draw the map and apply the vector data to the rastor in netlogo for the socioeconomic data
  ...
  gis:set-world-envelope (gis:envelope-union-of ;(gis:envelope-of sites)
    (gis:envelope-of notSeniorClinics)
    (gis:envelope-of seniorClinics)
    (gis:envelope-of censusBlocks)
    )

  ask patches [set pcolor white]

  gis:apply-coverage censusBlocks "GEO-ID" GEOID
  gis:apply-coverage censusBlocks "SeniorPOP" population
  gis:apply-coverage censusBlocks "edu" education
  gis:apply-coverage censusBlocks "pov" poverty
  gis:apply-coverage censusBlocks "fs" foodstamps
  gis:apply-coverage censusBlocks "disability" disability
  gis:apply-coverage censusBlocks "health_insurance" healthInsurance

  gis:apply-coverage censusBlocks "cancer" cancerValue
  gis:apply-coverage censusBlocks "bpHigh" highBloodPressure
  gis:apply-coverage censusBlocks "diabetes" diabetesValue
  ...
  gis:draw notSeniorClinics 2
;  mark-notSeniorClinics
  gis:set-drawing-color green
  gis:draw seniorClinics 3
;  mark-seniorClinics
...
;
;
;to mark-seniorClinics
;  ask patches[
;    if gis:intersects? seniorClinics self [set seniorClinic-here? TRUE]
;  ]
;end
;
;to mark-notSeniorClinics
;  ask patches[
;    if gis:intersects? notSeniorClinics self [set notSeniorClinic-here? TRUE]
;  ]
;end
;
;
